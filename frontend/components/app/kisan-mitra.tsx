...
// ==========================================
// KisanMitra Live Transcript & Chat Component
// ==========================================
function KisanMitraLiveTranscript() {
  ...
  return (
    <div className="mt-4 flex w-full flex-col rounded-3xl border border-emerald-300/80 bg-white/90 p-4 text-left shadow-xl backdrop-blur-xl md:p-6 dark:border-emerald-700/60 dark:bg-emerald-950/90">
      ...
      <div className="scrollbar-thin flex max-h-72 min-h-36 w-full flex-col space-y-3 overflow-y-auto rounded-2xl bg-emerald-50/50 p-3.5 dark:bg-[#021810]/60">
        {chatMessages.length === 0 ? (
          ...
          chatMessages.map((msg) => {
            const isFarmer = msg.from?.isLocal;
            return (
              <div
                key={msg.id}
                className={cn('flex w-full', isFarmer ? 'justify-end' : 'justify-start')}
              >
                <div
                  className={cn(
                    'max-w-[85%] rounded-2xl px-4 py-2.5 text-xs leading-relaxed font-medium shadow-xs md:text-sm',
                    isFarmer
                      ? 'rounded-tr-none bg-emerald-600 text-white'
                      : 'rounded-tl-none border border-emerald-300/80 bg-white text-emerald-950 dark:border-emerald-700/70 dark:bg-emerald-900 dark:text-emerald-100'
                  )}
                >
                  <div className="mb-1 flex items-center gap-1.5 text-[11px] font-bold opacity-80">
                    <span>{isFarmer ? '🧑‍🌾 आप (Farmer)' : '🌾 किसानमित्र AI'}</span>
                  </div>
        ...
        {(agentState === 'thinking' || isSending) && (
          <div className="flex justify-start">
            <div className="flex items-center gap-2 rounded-2xl rounded-tl-none border border-emerald-300/80 bg-white px-3.5 py-2 text-xs font-semibold text-emerald-900 shadow-xs dark:border-emerald-700 dark:bg-emerald-900 dark:text-emerald-100">
      ...
      {/* Inline Text Chat Input (Type Script & Written Replies) */}
      <form
        onSubmit={(e) => {
          e.preventDefault();
          handleSend();
      ...
      >
        <input
          ...
          placeholder="अपना सवाल यहाँ टाइप करें (उदा. गेहूं में खाद कब डालें)..."
          disabled={isSending}
...
export function KisanMitraAI({ appConfig }: KisanMitraAIProps) {
  ...
  // LiveKit Session Initialization
  ...
  return (
    <AgentSessionProvider session={session}>
      {/* Requirement 1: Background Theme */}
      <div className="text-foreground relative flex min-h-screen w-full flex-col justify-between overflow-x-hidden bg-gradient-to-b from-lime-50 via-emerald-50 to-lime-100 transition-colors duration-500 selection:bg-emerald-300 selection:text-emerald-950 dark:from-[#021810] dark:via-[#04261a] dark:to-[#021810]">
        ...
        <main className="flex grow flex-col items-center justify-between pb-12">
          ...
          <VoiceAgentController micError={micError} onRetryMic={checkMicrophonePermission} />
