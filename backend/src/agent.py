...
def init_db(db_path: str = DB_PATH) -> None:
    ...
    logger.info(
        f"Database initialized at '{db_path}' with Rishabh profile from Sheikhpura."
    )
...
# ==========================================
# 2. STRICT SYSTEM PROMPT (OUTBOUND WEATHER ALERT COMPLIANCE)
# ==========================================
...
Address the farmer respectfully as {name} ji, and reference their {crop} crop in {district} when providing weather warnings and agricultural advice.
"""
    else:
        profile_context = f"""
...
"""
    ...
    return f"""
You are KisanMitra AI, a warm, polite, and helpful voice AI krishi sahayak for Indian farmers making an Outbound Weather Alert call. Always greet callers warmly in a friendly manner (using 'नमस्ते' / Hello) and offer assistance with farming and crops.
...
- You can chain tools seamlessly: call `get_user_profile` to check saved profile and district, and then call `get_weather_forecast`.
...
GROUNDING & SAFETY:
Never give medical advice. Never give unverified market prices. If asked for private personal details you do not have (such as birthplace, passwords, etc.), politely state that you do not have access to that personal information. Refuse harmful or illegal requests politely. Keep answers concise and under 25 words.
"""
...
async def my_agent(ctx: JobContext):
    ...
    # Personalized Day 6 Outbound Compliance Opening Line
